...
mysqli_free_result($result);
 ...
 if(isset($_POST['add'])){
    ...
    //check if the records exist to insert or not
    $check = mysqli_query($connection,"SELECT * FROM nominee WHERE ncode = '$key' ") or die("not Added");
    
    if(mysqli_num_rows($check)>0){
        //if records found and inserted
    
        $insert = mysqli_query($connection,"INSERT INTO nomineev SELECT * FROM nominee WHERE ncode = '$key'") or die("Not Added");
